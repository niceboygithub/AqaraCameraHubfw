...
#
# Product model, support list: AH_M200
#
# AH_M100 : Aqara Hub M200.
#
...
KERNEL_MD5SUM="ed009abc7d71c5d63afc3975bdc95e25"
ROOTFS_MD5SUM="7d2de81221c20d19c198799699064977"
MUSICS_MD5SUM=""
MODIFIED_ROOTFS_MD5SUM="c26a58fdac23281d56eb404bb7beedb7"
...
usage_helper() {
    ...
    green_echo "Usage: m200_update.sh -h [$OPTIONS]."
}
...
usage_updater() {
    ...
    green_echo " -o : update original rootfs."
}
...
update_get_packages() {
    ...
    if [ "x${COOR_MD5SUM}" != "x" ]; then
        /tmp/curl -s -k -L -o /tmp/coor.bin ${FIRMWARE_URL}/original/${simple_model}/${VERSION}/${COOR_BIN}
...
thread_ota() {
    ...
    asetprop persist.sys.zgb_ver $ver_file
    return 0
}
...
update_start() {
    ...
    ota_kernel $kernel_bin_
    if [ $? -eq 1 ]; then
        echo "kernel ota fail"
        return 1
    fi

    ota_rootfs $rootfs_bin_
    if [ $? -eq 1 ]; then
