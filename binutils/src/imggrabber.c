...
int main(int argc, char **argv)
{
    ...
    if (strncmp(sModuleName, "SC2235", 6) == 0) {
        ...
        if (resolution == RESOLUTION_LOW) {
            fPtr = fopen("/proc/mstar/OMX/VMFE2/ENCODER_INFO/IBUF_pBuffer", "r");
            fLen = fopen("/proc/mstar/OMX/VMFE2/ENCODER_INFO/IBUF_nAllocLen", "r");
        } else {
            fPtr = fopen("/proc/mstar/OMX/VMFE0/ENCODER_INFO/IBUF_pBuffer", "r");
            fLen = fopen("/proc/mstar/OMX/VMFE0/ENCODER_INFO/IBUF_nAllocLen", "r");
        }
