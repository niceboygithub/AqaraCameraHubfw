...
int main(int argc, char **argv)
{
    ...
    fMN = fopen("/proc/mstar/ISP/SensorInfo/ModuleName", "r");
    fscanf(fMN, "%s", sModuleName);
    fclose(fMN);
    ...
    if (strncmp(sModuleName, "SC2235", 6) == 0) {
        res = RESOLUTION_LOW;
        ...
        if (res == RESOLUTION_LOW) {
            fPtr = fopen("/proc/mstar/OMX/VMFE2/ENCODER_INFO/OBUF_pBuffer", "r");
            fLen = fopen("/proc/mstar/OMX/VMFE2/ENCODER_INFO/OBUF_nAllocLen", "r");
        } else {
            fPtr = fopen("/proc/mstar/OMX/VMFE0/ENCODER_INFO/OBUF_pBuffer", "r");
            fLen = fopen("/proc/mstar/OMX/VMFE0/ENCODER_INFO/OBUF_nAllocLen", "r");
        }
    ...
    if (strncmp(sModuleName, "SC2235", 6) == 0) {
        ...
        if (res == RESOLUTION_LOW) {
            sprintf(filLenFile, "/proc/mstar/OMX/VMFE2/ENCODER_INFO/OBUF_nFilledLen");
            sprintf(timeStampFile, "/proc/mstar/OMX/VMFE2/ENCODER_INFO/OBUF_nTimeStamp");
        } else {
            sprintf(filLenFile, "/proc/mstar/OMX/VMFE0/ENCODER_INFO/OBUF_nFilledLen");
            sprintf(timeStampFile, "/proc/mstar/OMX/VMFE0/ENCODER_INFO/OBUF_nTimeStamp");
        }
...
//			fOut = open(FIFO_NAME_LOW, O_RDWR);
            if (fOut == NULL) {
...
//            fOut = open(FIFO_NAME_HIGH, O_RDWR);
            if (fOut == NULL) {
				fprintf(stderr, "Error opening fifo %s\n", FIFO_NAME_HIGH);
				return -1;
            }
